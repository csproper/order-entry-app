...
export function DetailItem({
  ...
  const [showOptions, setShowOptions] = useState(
    values.noshi_type !== "なし" || values.wrapping_type !== "なし"
  );
  ...
  return (
    ...
    >
      {/* ── ヘッダー：商品情報 + 金額 ── */}
      <div className="flex items-center gap-3 px-4 py-3 border-b bg-gray-50/50 rounded-t-lg">
        <div className="flex items-center gap-3 min-w-0">
          <span className="text-sm font-semibold text-gray-500 shrink-0">
            #{index + 1}
          </span>
          <span className="font-mono text-sm bg-white px-2 py-0.5 rounded border text-gray-600 shrink-0">
            {values.product_code}
          </span>
          <span className="text-sm font-medium truncate">{values.product_name}</span>
          {hasAnyError && (
            <span className="text-xs text-red-500 bg-red-50 px-2 py-0.5 rounded shrink-0">
              入力エラーあり
            </span>
          )}
        </div>
      ...
      <div className="p-4 space-y-5">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold text-gray-700">お届け先</span>
            <Button
              type="button"
              variant="outline"
              size="sm"
        ...
        <div>
          ...
          >
            ...
            {!showOptions && (values.noshi_type !== "なし" || values.wrapping_type !== "なし") && (
              <span className="text-xs text-blue-500 font-normal ml-1">設定あり</span>
            )}
          </button>
          {showOptions && (
